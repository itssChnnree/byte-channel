...
start_service() {
    ...
    if ps -p $XRAY_PID > /dev/null 2>&1; then
        ...
        if [ -s "$LOG_FILE" ]; then
            tail -20 "$LOG_FILE"
            echo ""
            print_info "开始实时日志监控 (按Ctrl+C停止)..."
            echo ""
            
            # 使用timeout限制tail命令，避免永久阻塞
            timeout 300 tail -f "$LOG_FILE" 2>/dev/null || echo "日志监控结束"
        else
...
main() {
    ...
    echo -e "2. 查看服务状态: systemctl status xray-core"
    echo -e "3. 停止服务: systemctl stop xray-core"
    echo -e "4. 重启服务: systemctl restart xray-core"
}
